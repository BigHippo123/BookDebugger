...
const TextAnalyzer = () => {
  ...
  // Process text into paragraphs using API data
  ...
  const findSimilarWords = async (word) => {
    ...
    try {
      // Use the top words that appear near this word as similar
      const wordInfo = stats.wordDict[word.toLowerCase()];
      if (!wordInfo || !wordInfo.embedding) return [];
      
      // Find other words with embeddings and calculate simple similarity
      const similarities = [];
  ...
  const handleFileUpload = async (e) => {
    ...
    try {
      // First read the file for display
      ...
      <div className="max-w-7xl mx-auto p-6 grid grid-cols-1 lg:grid-cols-3 gap-6">
        ...
        <div className="lg:col-span-1">
          <div className="bg-white rounded-lg shadow p-6 sticky top-24">
            ...
            {selectedParagraph !== null && processedText[selectedParagraph] && (
              <div className="mt-6 p-4 bg-blue-50 border-2 border-blue-300 rounded-lg">
                <h3 className="font-bold text-gray-800 mb-2">Paragraph {selectedParagraph + 1}</h3>
                <div className="text-sm space-y-1">
                  <div>Words: <span className="font-semibold">{processedText[selectedParagraph].wordCount}</span></div>
                  <div>Sentences: <span className="font-semibold">{processedText[selectedParagraph].sentenceCount}</span></div>
                  <div>Avg Length: <span className="font-semibold">{processedText[selectedParagraph].avgSentenceLength.toFixed(1)} words</span></div>
                </div>
              </div>
            )}
        ...
        <div className="lg:col-span-2">
          <div className="bg-white rounded-lg shadow p-6">
            ...
            <p className="text-sm text-gray-600 mb-4">
              Hover over words for details and embeddings. Click paragraphs for analysis.
              {stats.hasEmbeddings && <span className="text-green-600 ml-2">✓ Word2Vec enabled</span>}
            </p>
            
            <div className="space-y-4">
              {processedText && processedText.map((para, pIdx) => (
                <div
                  key={pIdx}
                    ...
                    {para.text.split(/\b/).map((segment, sIdx) => {
                      ...
                      if (cleanWord && cleanWord.length > 0) {
                        ...
                        return (
                          <span
                            key={sIdx}
                            className={`hover:bg-yellow-200 hover:font-semibold cursor-pointer transition-colors duration-150 ${
        ...
        >
          <div className="bg-gray-900 text-white rounded-lg shadow-xl p-4 mb-2 max-w-xs">
            ...
            <div className="text-sm space-y-1">
              ...
              {similarWords[hoveredWord] && similarWords[hoveredWord].length > 0 && (
                <div className="mt-3 pt-2 border-t border-gray-700">
                  <div className="font-semibold mb-2 text-gray-300">Similar Words:</div>
